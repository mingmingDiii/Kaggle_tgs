...
class MPSPNet2_NO_SCSE_WI_PSP2(nn.Module):
    ...
    def forward(self, x):
        ...
        e5 = self.psp(e5)#1024,16,16

        #f = self.center(e5)# ; print('f',f.size()) #256,8,8

        #f= self.drop_1(f)
        d7 = self.decoder_p1(e5,e4) #256,16,16
        d6 = self.decoder_p2(d7,e3) #64,16,16

        d5 = self.decoder5(d6,e2)# ; print('d5',d5.size())#32,32
        #d5 = self.drop_1(d5)
        ...
        f = torch.cat((
            ...
            F.upsample(d6, scale_factor=8, mode='bilinear', align_corners=False)
        ),1)
