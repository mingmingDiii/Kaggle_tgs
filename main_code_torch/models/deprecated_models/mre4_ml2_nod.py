...
class MRE4_ML2_NOD(nn.Module):
    def __init__(self,pretrained=True,reduction=2):
        ...
        self.hyper_logit1 = nn.Sequential(
            nn.Conv2d(256,64,kernel_size=3,padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64,1,kernel_size=1,padding=0)
        )
        self.hyper_logit2 = nn.Conv2d(64, 1, kernel_size=1, padding=0)
        self.hyper_logit3 = nn.Conv2d(64, 1, kernel_size=1, padding=0)
        self.hyper_logit4 = nn.Conv2d(64, 1, kernel_size=1, padding=0)
        self.hyper_logit5 = nn.Conv2d(64, 1, kernel_size=1, padding=0)
    ...
    def forward(self, x):
        ...
        hyper_logit1 = self.hyper_logit1(f)#8*8
        hyper_logit2 = self.hyper_logit2(d5)#16*16
        hyper_logit3 = self.hyper_logit3(d4)#32*32
        hyper_logit4 = self.hyper_logit4(d3)#64*64
        hyper_logit5 = self.hyper_logit5(d2)#128*128

        fuse_feature = torch.cat((
            seg_base_fuse,
            F.upsample(f_gap_fuse,scale_factor=128,mode='nearest')
        ),1)#128,128,128

        fuse_logit = self.fuse_logit(fuse_feature)#1,128,128

        return fuse_logit,class_logit,[hyper_logit1,hyper_logit2,hyper_logit3,hyper_logit4,hyper_logit5]
