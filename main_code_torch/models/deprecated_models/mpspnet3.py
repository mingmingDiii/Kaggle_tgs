...
from models.common import mextractors3

# remove scse and psp module
# using decoding (hyper column) in hcnet
# more like a densenet-unet
...
class MPSPNet3(nn.Module):
    def __init__(self, n_classes=1, sizes=(1, 2, 3, 6), psp_size=1024,backend='resnet34',
                 pretrained=True):
        super().__init__()
        self.densenet = mextractors3.densenet121(pretrained=pretrained)
        ...
        self.encoder5 = nn.Sequential(
            ...
            #self.densenet.features.transition4
        )#8,8
    ...
    def forward(self, x):
        ...
        d5 = self.decoder5(f,e4)# ; print('d5',d5.size())#64,8,8
        #d5 = self.drop_1(d5)
        d4 = self.decoder4(d5,e3)#; print('d4',d4.size())#16,16
        #d4 = self.drop_1(d4)
        d3 = self.decoder3(d4,e2)#; print('d3',d3.size())#32,32
        #d3 = self.drop_1(d3)
        d2 = self.decoder2(d3,e1)#; print('d2',d2.size()) #64,64
        d1 = self.decoder1(d2)  # ; print('d2',d2.size()) #128,128
